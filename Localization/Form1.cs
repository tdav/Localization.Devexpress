...
namespace Localization
{
    public partial class Form1 : DevExpress.XtraEditors.XtraForm
    {
        ...
        private TransModel Model { get; set; }
        ...
        public Form1()
        {
            ...
            Model = JsonConvert.DeserializeObject<TransModel>(json);
        ...
        private void ParseDevexpress_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
                        Program.log.Error($"ParseDevexpress_Click Error: {ee.Message}");
                        goto Tamom;
                    }
                }
        ...
        private void ParseYandex_Click(object sender, EventArgs e)
        {
            ...
            {
                Program.log.Error($"ParseYandex_Click Error: {ee.Message}");
            }
        }
        ...
        private void BtnSerializeDesir_Click(object sender, EventArgs e)
        {
            var json = File.ReadAllText(AppDomain.CurrentDomain.BaseDirectory + "list.json");
            var devxModel = JsonConvert.DeserializeObject<List<DevxModel>>(json);

            var transModel = new TransModel();
            transModel.Models = new List<Model>();

            foreach (var m in devxModel)
            {
                var list = m.Items.Distinct().Where(x => x.EnglishTranslation != "");
                foreach (var item in list)
                {
                    var model = new Model();
                    model.Name = item.Name;
                    model.English = item.EnglishTranslation;
                    model.Suggested = item.SuggestedTranslation.Value;
                    transModel.Models.Add(model);
                }
            }
            var jsModel = JsonConvert.SerializeObject(transModel, Formatting.Indented);

            File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + "model.json", jsModel);
        }

        private void btnSave_Click(object sender, EventArgs e)
        {
            var json = JsonConvert.SerializeObject(modelBindingSource.DataSource);
            Model = modelBindingSource.DataSource as TransModel;
            File.WriteAllText(AppDomain.CurrentDomain.BaseDirectory + "model.json", json);
